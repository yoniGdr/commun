...
void print_line_at(int x, int y, const char *s, const int value)
{
...
void print_line_int(int y, const char *s, const int value)
{
    std::stringstream ss;
    ss << "|" << s << std::setw(width - strlen(s) - strlen(std::to_string(value).c_str())) << std::setfill(' ') << "|";
    print_line_at(posx, posy+y, ss.str().c_str(), value);
}
